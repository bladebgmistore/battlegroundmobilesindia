...
function SitePanel({ settings, save }: { settings: SettingRow[]; save: (k: string, v: unknown) => void }) {
  ...
  const [featuredTitle, setFeaturedTitle] = useState(existing("featured_drop_title", "ELITE INVENTORY"));
  const [featuredImage, setFeaturedImage] = useState(existing("featured_drop_image", ""));
